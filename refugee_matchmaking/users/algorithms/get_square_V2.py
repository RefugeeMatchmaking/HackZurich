			...
			for friend_of_friend in scnd:
				...
				if s2>threshold: #If the score is higher than the threshold then we have found a triangluar match (3 people)
					if friend_of_friend in triangle:
						triangle[friend_of_friend].append((friend,s1+s2))
					else:
						triangle[friend_of_friend]=[(friend,s1+s2)]
				

	#Loop through the dictionary of triangles
	for fof in triangle:

		#If our friend_of_friend has more two friends or more then we have a square
		if len(triangle[fof])>1:
			...
			#Lets sum the scores of the top two friends
			new_score = toptwo[0][1] + toptwo[1][1]
